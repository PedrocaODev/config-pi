...
6. **Use the witness lifecycle.** The witness workflow lifecycle is `grill → /witness-new → /witness-apply → /witness-archive`. Grilling (the grilling skills) is the brainstorm entry — it replaces any exploration command and stress-tests the idea before it becomes a proposal (skip it for trivial, clear work; ceremony scales with risk). `/witness-new` writes the proposal and planning artifacts, `/witness-apply` implements through test-first slices with a verification gate, `/witness-archive` archives and syncs. No `opsx-*` commands (propose/apply/archive) are used in this workflow. This rule also governs a user who directly invokes a generated `opsx-*` command: route it to the witness equivalent (`/witness-new` for proposal, `/witness-apply` for implementation, `/witness-archive` for closeout).
7. **Use the issue workflow for issues.** When the user is working on an issue (mentions one, pastes a URL or `#N` reference, or wants to refine, scope, implement, or close one), load the `issue-workflow` skill: Phase 1 refines the issue with a grilling session, Phase 2 drives the refined understanding through the `witness` OpenSpec schema (proposal → design → specs → tasks → plan → apply → verify → retrospective). Do not start Phase 2 until the user confirms the shared understanding reached in Phase 1.
...
- **Gates are deterministic.** Ask the `runner` Agent to execute the repository's verification command and report `PASSED`, `FAILED`, or `BLOCKED`. Only `PASSED` advances witness-apply verification and witness-archive. Use the repository's `verify.md` and `review.md` as the durable verification and review records. The integrator may merge only after explicit user approval and must never force-push.
...
* Load `issue-workflow` when the user is working on an issue: it runs `grill-me` to refine the issue, then drives the `witness` OpenSpec schema (lifecycle routing per Main Clause 6).
