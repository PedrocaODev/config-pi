---
description: Non-destructively adopt an existing OpenSpec repo into the global witness workflow.
---

# witness-adopt

Point an existing OpenSpec repository at the global `witness` schema
...
2. **Verify the global schema exists.**
   ...
   present, and that it resolves: `openspec schema which witness` →
   `~/.local/share/openspec/schemas/witness`. If not, stop and explain
...
3. **Initialize or update the project without tooling.**
   ...
   the global `/witness-*` commands drive the workflow.

4. **Set the schema to witness.**
   Update `openspec/config.yaml` to set `schema: witness`. Preserve all
   other existing config keys.

5. **Verify the schema resolves for this project.**
   Confirm the project now uses witness. Do not remove or replace
...
7. **Report result.**
   Tell the user the repo now uses witness for new changes and existing
   ...
   with `/witness-new`, implementation with `/witness-apply`, and archive with
   `/witness-archive`.
