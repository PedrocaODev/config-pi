---
name: clonedeps
description: Clone important project dependency source code into an ignored local workspace so pi can inspect library internals. Use when the user asks to clone dependencies, inspect dependency/source internals, understand SDK/framework behavior from source, debug library implementation details, or make core dependency repos locally readable. Do not use for ordinary API/docs questions where the `librarian` subagent is enough.
---

# Clonedeps Skill

You help users make a small set of important dependency source repositories
locally readable to pi.
...
orchestrator and the `librarian` subagent do the repo-specific thinking; the orchestrator
...
First check whether `.pi/clonedeps.json` exists.

If it exists:

1. Read it before asking librarian for a new plan.
2. Check whether each listed `path` exists under `.pi/clonedeps/repos/`.
...
Delegate dependency discovery and source resolution to the `librarian` subagent.
...
while the `.ignore` allowlist keeps them readable to pi.
...
```text
.pi/clonedeps/repos/<safe-repo-name>/
```
...
4. Clone into a temporary directory under `.pi/clonedeps/repos/`, then move it
...
Do not add `.pi/clonedeps.json` to `.gitignore`. It is small, reviewable
project metadata that can be committed. Only the cloned repository contents
under `.pi/clonedeps/repos/` should be ignored.
...
`.pi/clonedeps/repos/` for inspection. Do not edit these clones.

- `.pi/clonedeps/repos/<safe-name>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
- `.pi/clonedeps/repos/<safe-name-2>/` - `<repo>` at `<ref>`; <one sentence on
  why this source is useful>.
```

Also keep `.pi/clonedeps.json` updated as the structured manifest, but do not
...
Ask before removing `.pi/clonedeps.json` or the `AGENTS.md` section because
