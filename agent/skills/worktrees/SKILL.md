---
name: worktrees
description: Manage Git worktrees as pi safe isolated coding lanes for complex, risky, or parallel work.
---

# Worktrees Orchestration Protocol

The `worktrees` skill provides an opinionated, safe orchestration protocol for
managing Git worktrees as isolated coding lanes. Its value is giving the Orchestrator a consistent roster workflow for parallel agents, risky experiments, integration review, and cleanup.

## Core Contract

This is an **orchestrator-only** workflow. Other specialists such as the `fixer` subagent
or the `designer` subagent can be assigned tasks inside a worktree lane, but the
...
### State Tracking (`.pi/worktrees.json`)

Use the optional local metadata manifest `.pi/worktrees.json` to maintain
structural tracking:
  ...
  "lanes": [
    {
      ...
      "branch": "pi/feature-auth-v2",
      "path": ".pi/worktrees/feature-auth-v2",
      "base": "main",
...
If `.pi/worktrees.json` does not exist, create it when initializing a lane,
...
- Ensure the branch name (e.g. `pi/<slug>` or custom project convention) does not already exist locally or on remote.
- Ensure `.pi/worktrees/` is ignored by Git before creating nested worktrees.
...
2. Formulate a branch name. Default to `pi/<slug>` unless project/user conventions dictate otherwise.
...
5. Run:
   ```bash
   git worktree add -b <branch-name> .pi/worktrees/<slug> <base-commit/branch>
   ```
6. Register the metadata in `.pi/worktrees.json`.

### Phase 2: Execution & Delegation
1. Run all sub-agents with their working directory set strictly to the worktree
   path, such as `.pi/worktrees/<slug>`.
...
5. Safely remove the worktree using:
   ```bash
   git worktree remove .pi/worktrees/<slug>
   ```
6. Update `.pi/worktrees.json` to mark the lane as `archived` or remove it.
